...
import com.pwlsj.chat.adapter.FirstChargeRecyclerAdapter;
import com.pwlsj.chat.base.BaseCompactFragment;
import com.pwlsj.chat.base.BaseListResponse;
import com.pwlsj.chat.bean.FirstChargeRankBean;
...
public class FirstChargeFragment extends BaseCompactFragment implements View.OnClickListener {
    ...
    protected void initView(View view, Bundle savedInstanceState) {
        ...
        mAdapter = new FirstChargeRecyclerAdapter(mContext);
    ...
    private void getRankList(int queryType) {
        ...
        OkHttpUtils.post().url(ChatApi.GET_FIRST_CHARGE())
                .addParams("param", ParamUtil.getParam(paramMap))
                .build().execute(new AjaxCallback<BaseListResponse<FirstChargeRankBean>>() {
            @Override
            public void onResponse(BaseListResponse<FirstChargeRankBean> response, int id) {
                if (response != null && response.m_istatus == NetCode.SUCCESS) {
                    List<FirstChargeRankBean> focusBeans = response.m_object;
                    if (focusBeans != null) {
