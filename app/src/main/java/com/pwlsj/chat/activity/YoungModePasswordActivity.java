...
import com.pwlsj.chat.view.PwdEditText;
...
public class YoungModePasswordActivity extends BaseActivity {
    ...
    private void doOption() {
        ...
        if (password.length() < 4) {
            ToastUtil.showToast(getApplicationContext(), R.string.please_input_long_password);
            return;
        }
        ...
        if (text.equals(getString(R.string.next_step))) {//下一步
            ...
            if (!TextUtils.equals(mLastPassword, password)) {
                ToastUtil.showToast(getApplicationContext(), R.string.password_wrong);
                return;
            }
            //保存密码
            ...
            if (!TextUtils.equals(savePassword, password)) {
                ToastUtil.showToast(getApplicationContext(), R.string.wrong_password);
                return;
            }
            //输入正确的话, 删掉保存的
