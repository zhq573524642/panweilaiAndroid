...
public class SystemMessageActivity extends BaseActivity {
    ...
    private final List<SystemMessageBean> mFocusBeans = new ArrayList<>();
    ...
    protected void onContentAdded() {
        ...
        initRecycler();
        getMessageList(mRefreshLayout, true, 1);
        setHaveRead();
    }
     ...
     * 获取消息列表
     */
    private void getMessageList(final RefreshLayout refreshlayout, final boolean isRefresh, int page) {
        Map<String, String> paramMap = new HashMap<>();
        paramMap.put("userId", getUserId());
        paramMap.put("page", String.valueOf(page));
        OkHttpUtils.post().url(ChatApi.GET_MESSAGE_LIST())
                .addParams("param", ParamUtil.getParam(paramMap))
                .build().execute(new AjaxCallback<BaseResponse<PageBean<SystemMessageBean>>>() {
            @Override
            public void onResponse(BaseResponse<PageBean<SystemMessageBean>> response, int id) {
                if (isFinishing()) {
                    return;
                }
                if (response != null && response.m_istatus == NetCode.SUCCESS) {
                    PageBean<SystemMessageBean> pageBean = response.m_object;
                    if (pageBean != null) {
                        List<SystemMessageBean> focusBeans = pageBean.data;
                        if (focusBeans != null) {
    ...
    private void initRecycler() {
        mRefreshLayout.setOnRefreshListener(new OnRefreshListener() {
            ...
            public void onRefresh(@NonNull RefreshLayout refreshlayout) {
                getMessageList(refreshlayout, true, 1);
            }
        });
        mRefreshLayout.setOnLoadMoreListener(new OnLoadMoreListener() {
            @Override
            public void onLoadMore(@NonNull RefreshLayout refreshlayout) {
                getMessageList(refreshlayout, false, mCurrentPage + 1);
            }
        });

        LinearLayoutManager gridLayoutManager = new LinearLayoutManager(this);
        mContentRv.setLayoutManager(gridLayoutManager);
        mAdapter = new SystemMessageRecyclerAdapter(this);
