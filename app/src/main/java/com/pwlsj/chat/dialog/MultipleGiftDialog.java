...
import com.pwlsj.chat.view.recycle.OnViewPagerListener;
import com.pwlsj.chat.view.recycle.ViewHolder;
import com.pwlsj.chat.view.recycle.ViewPagerLayoutManager;
    ...
    private void setGiftDialogView() {
        ...
        reward_tv.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (selectedSize == 0) {
                    ToastUtil.showToast(activity, "请点击头像，选择受赠礼物的主播");
                    return;
                }
                GiftBean giftBean = giftAdapter.getSelectBean();
                if (giftBean == null) {
                    ToastUtil.showToast(activity, R.string.please_select_gift);
                    return;
                }
                if (selectedSize * giftBean.t_gift_gold > mMyGoldNumber) {
                    ToastUtil.showToast(activity, R.string.gold_not_enough);
                    return;
                }
            ...
            public void onResponse(BaseResponse response, int id) {
                ...
                if (response != null) {
                    if (response.m_istatus == NetCode.SUCCESS) {
                        ToastUtil.showToast(activity, activity.getString(R.string.reward_success));
                        dismiss();
                        sendOk(giftBean, 0, list);
                    } else if (response.m_istatus == -1) {
                        ToastUtil.showToast(activity, R.string.gold_not_enough);
                    } else {
                        ToastUtil.showToast(activity, activity.getString(R.string.pay_fail));
                    }
                } else {
                    ToastUtil.showToast(activity, activity.getString(R.string.pay_fail));
                }
