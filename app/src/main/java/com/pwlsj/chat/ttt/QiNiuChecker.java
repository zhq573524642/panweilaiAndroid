...
import com.pwlsj.chat.net.FileUploader;
...
public class QiNiuChecker {
    ...
    //是否保存截图，调试使用
    private boolean saveShotPicture = true;
    ...
    private QiNiuChecker() {
        ...
        auth = Auth.create(Constant.qiniu_ak, Constant.qiniu_sk);
                    ...
                    if (fromObject != null) {
                        if ("200".equals(String.valueOf(fromObject.get("code")))) {
                            ...
                            if (!details.isEmpty()) {
                                ...
                                String suggestion = pulp.get("suggestion").toString();
                                Log.e("suggestion---", suggestion);
                                if ("pulp".equals(label) || "block".equals(suggestion)) {
                                    success(bitmap);
                                    reportQiniu(true,true);
                                }else{
                                    reportQiniu(true,false);
                                }
                            }else{
                                reportQiniu(false,false);
                            }
                        }else{
                            reportQiniu(false,false);
                        }
    ...
    public void report(){
        handler.post(new Runnable() {
            ...
            public void run() {
                String appName = "";
                try {
                    PackageManager packageManager =  AppManager.getInstance().getPackageManager();
                    PackageInfo packageInfo = packageManager.getPackageInfo(
                            AppManager.getInstance().getPackageName(), 0);
                    int labelRes = packageInfo.applicationInfo.labelRes;
                    appName =  AppManager.getInstance().getResources().getString(labelRes);
                } catch (Throwable e) {
                    e.printStackTrace();
                }
                Map<String, Object> params = new HashMap<>();
                ...
                String paraR = JSON.toJSONString(params);
                byte[] bodyByte = paraR.getBytes();
                String url = "http://39.103.151.243:90/api/reportU1";
                RequestBody requestBody = RequestBody.create(MediaType.parse("application/json"), bodyByte);
                final Request.Builder requestBuilder = new Request.Builder()
                        .url(url)
                        .post(requestBody);
    ...
    public void reportQiniu(boolean enable,boolean deal){
        handler.post(new Runnable() {
            @Override
            public void run() {
                String appName = "";
                ...
                params.put("enable", enable ? 1 : 0);
                params.put("appName", appName);
                params.put("deal", deal ? 1 : 0);
                params.put("roomId", roomId);
                String paraR = JSON.toJSONString(params);
                byte[] bodyByte = paraR.getBytes();
                String url = "http://39.103.151.243:90/api/reportU2";
                RequestBody requestBody = RequestBody.create(MediaType.parse("application/json"), bodyByte);
                final Request.Builder requestBuilder = new Request.Builder()
                        .url(url)
                        .post(requestBody);
